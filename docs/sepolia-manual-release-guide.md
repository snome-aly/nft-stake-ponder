...
Build Command: node .yarn/releases/yarn-3.2.3.cjs install --immutable && node .yarn/releases/yarn-3.2.3.cjs ponder:typecheck
Start Command: node .yarn/releases/yarn-3.2.3.cjs ponder:start
```
...
- Railway 的 Railpack 环境有时 `corepack prepare yarn@3.2.3 --activate` 后仍然找不到 `yarn` 命令，所以这里直接调用仓库提交的 `.yarn/releases/yarn-3.2.3.cjs`。
