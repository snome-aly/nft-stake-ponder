...
Build Command: corepack prepare yarn@3.2.3 --activate && corepack yarn install --immutable && corepack yarn ponder:typecheck
Start Command: corepack prepare yarn@3.2.3 --activate && DATABASE_SCHEMA=${DATABASE_SCHEMA:-public} corepack yarn ponder:start
```
...
- 不要把 Root Directory 设置成 `packages/ponder`。这个项目是 monorepo，Ponder 会读取 `packages/nextjs/contracts/deployedContracts.ts`，并且 Yarn 配置、lockfile 都在仓库根目录。
- 如果 Railway 已经有 Root Directory 是 `packages/ponder` 的服务，进入服务的 Settings，把 Root Directory 清空或改成仓库根目录，然后重新部署。
- Railway 的 Railpack 环境有时 `corepack prepare yarn@3.2.3 --activate` 后仍然找不到 PATH 里的 `yarn` 命令，所以这里使用 `corepack yarn` 显式调用。
- `DATABASE_SCHEMA` 是 Ponder 生产模式需要的数据库 schema。独立数据库可以用 `public`；如果多个服务共用同一个 Postgres，建议改成唯一值，例如 `nft_stake_ponder`。
